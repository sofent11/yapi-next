...
| Import/export | Bruno, Postman, OpenAPI, Insomnia, WSDL, OpenCollection, env | Partial | Existing OpenAPI/Postman/HAR remain. Bruno single `.bru` HTTP request import/export now maps method/url/query/path params/headers/body/auth/scripts/tests/docs across the local workspace model, with a response toolbar copy entry for the active request. Selected collections can now export a Bruno folder (`bruno.json`, `collection.bru`, `folder.bru`, request `.bru` files) from the Collection Runner. Multi-file Bruno collection import and Insomnia/WSDL/OpenCollection are pending. |
...
4. Add Bruno multi-file collection import/export plus Insomnia fixtures and round-trip tests.
