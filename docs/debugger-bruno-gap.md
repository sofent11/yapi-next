...
| Variables | Collection/folder/request/response/env/global/prompt vars | Partial | Schema has scoped variable rows. Request-scoped variable rows now resolve through preview/runtime, and prompt-scoped request rows ask for values right before send/auth refresh. Runtime also keeps resolving project/env/runtime/data/step sources. Folder-scoped editing and broader prompt-variable management are still pending. |
...
| Response viewer | JSON/XML/HTML/text/binary/media/PDF, search, download | Partial | Existing body/json/headers/cookies/compare/raw remain. The debugger now keeps a base64 copy of live response bytes, exposes a dedicated preview tab, renders HTML, image, audio, video, PDF, and binary-oriented previews based on content type, and allows direct payload download. Structured search and deeper diff tooling are still pending. |
