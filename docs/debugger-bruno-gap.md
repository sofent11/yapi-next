...
| Request kinds | HTTP, GraphQL, gRPC, WebSocket, JS | Partial | Schema has `kind: http/graphql/grpc/websocket/script`; GraphQL sends over HTTP POST. WebSocket has batch connect/send/receive plus live connect/send/close controls, persists the most recent timeline on the request, and supports json/text/base64 binary outgoing frames with base64 previews for inbound binary frames. Reconnect UX, saved examples, and gRPC/script item runtimes are pending. |
...
2. Expand WebSocket runtime polish: reconnect UX, richer saved examples, and response/body previews for live sessions.
