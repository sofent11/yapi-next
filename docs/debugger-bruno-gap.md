...
| Import/export | Bruno, Postman, OpenAPI, Insomnia, WSDL, OpenCollection, env | Partial | Existing OpenAPI/Postman/HAR remain. Bruno single `.bru` HTTP request import/export maps method/url/query/path params/headers/body/auth/scripts/tests/docs across the local workspace model. Selected collections can export a Bruno folder (`bruno.json`, `collection.bru`, `folder.bru`, request `.bru` files), and the import panel can now read a Bruno collection folder back into requests plus a runnable YAPI Collection. Insomnia/WSDL/OpenCollection are pending. |
