...
| Variables | Collection/folder/request/response/env/global/prompt vars | Partial | Schema has scoped variable rows. Request-scoped variable rows now resolve through preview/runtime, prompt-scoped request rows ask for values right before send/auth refresh, and category folders now persist `_folder.yaml` variable documents that feed request preview/runtime plus collection runs with parent-folder inheritance and child-folder override behavior. Broader prompt-variable management is still pending. |
...
| Response viewer | JSON/XML/HTML/text/binary/media/PDF, search, download | Partial | Existing body/json/headers/cookies/compare/raw remain. The debugger now keeps a base64 copy of live response bytes, exposes a dedicated preview tab, renders HTML, image, audio, video, PDF, and binary-oriented previews based on content type, allows direct payload download, adds response-wide search across body/JSON/headers/cookies, and surfaces line-level compare snippets for live-vs-example diffs. Full-text highlighting and richer diff workflows are still pending. |
...
1. Reassess remaining partial parity items after the folder variable milestone: Bruno JS API parity, broader prompt-variable management, runner presets, richer diff workflows, and broader UI density polish.
2. Decide whether the next parity pass should prioritize scripting depth or response compare polish.
