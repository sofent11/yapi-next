...
| Runner | GUI runner, tags, env matrix, reports | Partial | Existing collection runner supports iteration, env matrix, retries, reports. GUI and CLI can filter runs by tags across step/request/case metadata, reports expose active filters, and failed reruns inherit report filters. Broader CLI packaging/report presets are pending. |
