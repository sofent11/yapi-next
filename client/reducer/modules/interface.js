...
export async function fetchInterfaceListMenu(projectId) {
  ...
  try {
    ...
    let list = [];
    do {
      // 新接口：按分类分页拉取，避免超大项目一次性返回过大
      const response = await axios.get('/api/interface/tree', {
        params: {
      ...
      if (!response || !response.data || response.data.errcode !== 0) {
        throw new Error('tree api failed');
      }
    ...
    // 兼容老后端：回退到旧接口
    result = await axios.get('/api/interface/list_menu?project_id=' + projectId);
  }
  return {
