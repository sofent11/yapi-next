...
export type AutoTestReportLike = {
  message: ReportMessage;
  runTime: string;
  list: ReportItem[];
};

const defaultTheme = `<style>${DEFAULT_THEME_CSS}</style>`;

function jsonFormat(input: unknown): string {
...
function codeHtml(code: number): string {
  switch (code) {
    case 0:
      return '<div title="验证通过" class="status status-ok"><i class="icon icon-check-circle"></i></div>';
    case 400:
      return '<div title="请求异常" class="status status-ko"><i class="icon icon-close-circle"></i></div>';
    case 1:
      return '<div title="验证失败" class="status status-warning"><i class="icon icon-warning-circle"></i></div>';
    default:
      return '<div title="验证通过" class="status status-warning"><i class="icon icon-warning-circle"></i></div>';
  }
}

function leftHtml(index: number, name: string, code: number): string {
  return `
...
function baseHtml(index: number, name: string, path: string, status?: number | null): string {
  return `
  <div>
    <h2 id=${index}>${name}</h2>
    <h3>基本信息</h3>
...
function validHtml(validRes?: Array<{ message: string }>): string {
  const message = Array.isArray(validRes)
    ? validRes.map(item => `<div>${item.message}</div>`).join('')
    : '';
  return `
  <div>
    <div class="row case-report">
...
function requestHtml(url?: string, headers?: unknown, params?: unknown): string {
  const formattedHeaders = jsonFormat(headers);
  const formattedParams = jsonFormat(params);
...
function responseHtml(resHeader?: unknown, resBody?: unknown): string {
  const formattedHeader = jsonFormat(resHeader);
  const formattedBody = jsonFormat(resBody);
  let html = '<div><h3>Reponse</h3>';

  html += formattedHeader
    ? `<div class="row case-report">
   <div class="col-3 case-report-title">Headers</div>
   <div class="col-21">
...
function createHtml5(left: string, content: string, msg: ReportMessage, runTime: string): string {
  const message =
    msg.failedNum === 0
      ? `<div>一共 <span class="success">${msg.successNum}</span> 测试用例， 全部验证通过(${runTime})</div>`
      : `<div>一共 ${msg.len} 测试用例，<span class="success"> ${msg.successNum}</span> 个验证通过， ${msg.failedNum} 个未通过(${runTime})</div>`;
...
function createHtml(report: AutoTestReportLike): string {
  let content = '';
  let left = '';
  for (const [index, item] of report.list.entries()) {
    content += baseHtml(index, item.name, item.path, item.status);
    content += validHtml(item.validRes);
    content += requestHtml(item.url, item.headers, item.data);
    content += responseHtml(item.res_header, item.res_body);
    left += leftHtml(index, item.name, item.code);
  }
  return createHtml5(left, content, report.message, report.runTime);
}

export function renderReportHtml(report: AutoTestReportLike): string {
  return createHtml(report);
}
