...
export function CompatWorkbenchPage() {
  ...
  async function handleTestPost() {
    const response = await testPost({ source: 'compat-workbench', action: 'post' }).unwrap();
    setTestResultText(pretty(response));
  }

  async function handleTestPut() {
    const response = await testPut({ source: 'compat-workbench', action: 'put' }).unwrap();
    setTestResultText(pretty(response));
  }

  async function handleTestPatch() {
    const response = await testPatch({ source: 'compat-workbench', action: 'patch' }).unwrap();
    setTestResultText(pretty(response));
  }

  async function handleTestDelete() {
    const response = await testDelete({ source: 'compat-workbench', action: 'delete' }).unwrap();
    setTestResultText(pretty(response));
  }
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card title="兼容接口迁移工作台">
        <Paragraph type="secondary" style={{ marginBottom: 0 }}>
          覆盖本轮新增迁移接口：`open/run_auto_test`、`log/list`、`log/list_by_update`、`test/*`、`interface/interUpload`。
        </Paragraph>
      </Card>
