...
(async () => {
  try {
    // 1) 先连接数据库（db.js 里会在连接成功后执行 autoIncrement.initialize(...)）
    const dbPromise = dbModule.connect();
    // 兼容老插件：它们期望 yapi.connect 是一个 thenable
    yapi.connect = dbPromise;
    await dbPromise;

    // 2) 再加载会触发 Schema.plugin(...) 的模块
    ...
    commons.log(`Fatal: ${err && err.stack || err}`, 'error');
    process.exit(1);
  }
})();
