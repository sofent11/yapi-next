...
function routeIndexKey(projectId, method, version) {
  return `${projectId}:${String(method || '').toUpperCase()}:${version || 0}`;
}

function clearRouteIndexByPrefix(projectId, method) {
  const prefix = `${projectId}:${String(method || '').toUpperCase()}:`;
  for (const key of routeIndexCache.keys()) {
    if (key.indexOf(prefix) === 0) {
      routeIndexCache.delete(key);
    }
  }
}

async function getRouteIndex(interfaceInst, projectId, method, projectVersion) {
  const key = routeIndexKey(projectId, method, projectVersion);
  if (routeIndexCache.has(key)) {
    return routeIndexCache.get(key);
  }

  clearRouteIndexByPrefix(projectId, method);
  const all = await interfaceInst.listByProjectAndMethod(
    projectId,
  ...
  all.forEach(item => {
    ...
    const exactPath = item.path;
    if (!staticMap.has(exactPath)) {
      staticMap.set(exactPath, []);
    }
    staticMap.get(exactPath).push(item);

    const queryPath = item.query_path && item.query_path.path;
    if (queryPath) {
...
module.exports = async (ctx, next) => {
  ...
  try {
    ...
    const routeIndex = await getRouteIndex(interfaceInst, project._id, ctx.method, project.up_time);
    interfaceData = routeIndex.staticMap.get(newpath) || [];
    let queryPathInterfaceData = routeIndex.queryPathMap.get(newpath) || [];
    ...
    if (!interfaceData || interfaceData.length === 0) {
      let newData = routeIndex.varList || [];
