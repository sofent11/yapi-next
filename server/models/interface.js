...
class interfaceModel extends baseModel {
  ...
  listByProjectAndMethod(project_id, method, select) {
    return this.model
      .find({
        project_id: project_id,
        method: method
  ...
  listByProjectForMenu(project_id, status) {
    const option = { project_id: project_id };
    if (status === 'open') {
    ...
    return this.model
      .find(option)
      .select(
        '_id title uid path method project_id catid edit_uid status add_time up_time index tag api_opened'
      )
      .sort({ catid: 1, index: 1, title: 1 })
      .exec();
  }

  listByProjectForExport(project_id, status) {
    const option = { project_id: project_id };
    if (status === 'open') {
      option.api_opened = true;
    }
