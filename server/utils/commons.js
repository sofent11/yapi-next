...
exports.log = (msg, type) => {
  ...
  fs.writeFile(
    ...
    err => {
      if (err) {
        console.error('error:', err && err.message ? err.message : err); // eslint-disable-line
      }
    }
  );
};
...
function getCompiledValidator(schemaDef) {
  let cacheKey;
  try {
    cacheKey = JSON.stringify(schemaDef || {});
  } catch (e) {
    cacheKey = null;
  }

  if (cacheKey && validateCache.has(cacheKey)) {
    return validateCache.get(cacheKey);
  }

  const schemaInput = Object.assign({}, schemaDef);
  const closeRemoveAdditional = schemaInput.closeRemoveAdditional === true;
  delete schemaInput.closeRemoveAdditional;

  const ajv = new Ajv({
    allErrors: true,
    coerceTypes: true,
    useDefaults: true,
    removeAdditional: closeRemoveAdditional ? false : true
  });
  const schema = ejs(schemaInput);
  schema.additionalProperties = closeRemoveAdditional ? true : false;
  const compiled = {
    ajv,
    validate: ajv.compile(schema)
  };
  if (cacheKey) {
    validateCache.set(cacheKey, compiled);
  }
  return compiled;
}

exports.validateParams = (schema2, params) => {
  const compiled = getCompiledValidator(schema2 || {});
  let valid = compiled.validate(params);

  let message = '请求参数 ';
  if (!valid) {
    ajvI18n.zh(compiled.validate.errors);
    message += compiled.ajv.errorsText(compiled.validate.errors, { separator: '\n' });
  }
