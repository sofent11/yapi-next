...
class interfaceController extends baseController {
  ...
  async listByMenu(ctx) {
    ...
    const recordMetric = status => {
      metrics.incCounter('yapi_interface_list_menu_requests_total', { status }, 1);
      metrics.observeHistogram('yapi_interface_list_menu_duration_ms', Date.now() - startedAt, { status });
    };
    let project_id = ctx.params.project_id;
    if (!project_id) {
      recordMetric('reject');
    ...
    try {
      const [catList, interfaceList] = await Promise.all([
        this.catModel.list(project_id),
        this.Model.listByProjectForMenu(project_id)
      ]);
      ...
      interfaceList.forEach(inter => {
        ...
        interfaceMap.get(key).push(item);
      });

      const newResult = catList.map(cat => {
  ...
  async tree(ctx) {
    ...
    const limit = parseInt(ctx.params.limit || 20, 10);
    const status = ctx.params.status;
    const tag = ctx.params.tag;
    const includeList = ctx.params.include_list === true;
    ...
    try {
      const allCats = await this.catModel.list(project_id);
      const total = allCats.length;
      const safePage = page > 0 ? page : 1;
      const safeLimit = limit > 0 ? limit : 20;
      const start = (safePage - 1) * safeLimit;
      const pageCats = allCats.slice(start, start + safeLimit);
      ...
      const option = { project_id, catid: { $in: catIds } };
      if (status) {
      ...
      const countRows = await this.Model.model.aggregate([
        { $match: option },
        { $group: { _id: '$catid', count: { $sum: 1 } } }
      ]);
      const countMap = new Map();
      countRows.forEach(item => countMap.set(item._id, item.count));

      const map = new Map();
      if (includeList) {
        const interfaces = await this.Model.listByOptionWithPage(option, 1, Math.max(2000, safeLimit * 200));
        interfaces.forEach(item => {
          const data = item.toObject();
          const key = data.catid;
          if (!map.has(key)) {
            map.set(key, []);
          }
      ...
      const list = pageCats.map(cat => {
        const data = cat.toObject();
        const current = map.get(data._id) || [];
        data.interface_count = countMap.get(data._id) || 0;
        if (includeList) {
          data.list = current;
        }
        return data;
  ...
  async treeNode(ctx) {
    ...
    const page = parseInt(ctx.params.page || 1, 10);
    const limit = parseInt(ctx.params.limit || 50, 10);
    const status = ctx.params.status;
    const tag = ctx.params.tag;
    ...
    try {
      ...
      if (!catData) {
        return (ctx.body = yapi.commons.resReturn(null, 404, '不存在的分类'));
      }
      ...
      const list = await this.Model.listByOptionWithPage(option, page, limit);
  ...
  async listByOpen(ctx) {
    ...
    try {
      const result = await this.Model.listByProjectForMenu(project_id, 'open');
