...
class openController extends baseController {
  ...
  async importData(ctx) {
    ...
    const recordMetric = status => {
      metrics.incCounter('yapi_open_import_data_requests_total', { status, type, dataSync }, 1);
      metrics.observeHistogram('yapi_open_import_data_duration_ms', Date.now() - startedAt, {
        status,
        type,
        dataSync
      });
    ...
    try {
      if (ctx.params.url) {
        const response = await axios.get(ctx.params.url, { timeout: 30000 });
        content = response.data;
      } else if (
        typeof content === 'string' &&
        (content.indexOf('http://') === 0 || content.indexOf('https://') === 0)
      ) {
        const response = await axios.get(content, { timeout: 30000 });
        content = response.data;
      }
    ...
    if (!res || typeof res !== 'object' || !Array.isArray(res.apis)) {
      recordMetric('error');
      return (ctx.body = yapi.commons.resReturn(null, 40022, '导入数据解析失败'));
    }
    ...
    const requestCookie =
      ...
      (this.ctx && this.ctx.request && this.ctx.request.header && this.ctx.request.header.cookie) ||
      '';
